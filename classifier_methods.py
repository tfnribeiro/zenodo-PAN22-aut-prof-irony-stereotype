...
def get_features_train(author_list_train, emoji_pca_dim=5, profanity_pca_dim=10, word_pca_dim=20, label="Generating Train Features", supress_prints_flag=False):
    ...
    emoji_tfidf_features = get_features(
        author_list_train, emoji_tfidf.tf_idf, "Emoji TF_IDF")

    profanity_tfidf = fit_profanity_embeds_tfidf(
        author_list_train, authors_document=False)
    profanity_tfidf_features = get_features(
        author_list_train, profanity_tfidf.tf_idf, "Profanity TF_IDF")

    words_tfidf = fit_word_embeds_tfidf(
        author_list_train, authors_document=False)
    words_tfidf_features = get_features(
        author_list_train, words_tfidf.tf_idf, "Words TF_IDF")

    emoji_features_train = emoji_pca.fit_transform(emoji_tfidf_features)
    print("Emoji Explained Variance: ", sum(emoji_pca.explained_variance_ratio_))
    profanity_features_train = profanity_pca.fit_transform(
        profanity_tfidf_features)
    print("Profanity Explained Variance: ", sum(profanity_pca.explained_variance_ratio_))
...
def cross_validate(X, y, split_n=7, emoji_pca_dim=5, profanity_pca_dim=10, word_pca_dim=20,
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X)):
        ...
        if i > 0:
            print(f"{stop_w.current_time()}: Split took {stop_w.split(as_string=True)}")
        print(
        ...
        X_test = get_features_test(
            X_test, emoji_pca, profanity_pca, word_pca, emoji_tfidf, profanity_tfidf, words_tfidf)
