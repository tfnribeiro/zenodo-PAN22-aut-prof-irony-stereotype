...
def get_features_train(author_list_train, emoji_pca_dim = 5, profanity_pca_dim = 10, word_pca_dim = 20, label="Generating Train Features", supress_prints_flag=False):
    ...
    emoji_tfidf = fit_emoji_embeds_tfidf(author_list_train,authors_document=False)
    emoji_tfidf_features = get_features(author_list_train, emoji_tfidf.tf_idf, "Emoji TF_IDF")

    profanity_tfidf = fit_profanity_embeds_tfidf(author_list_train,authors_document=False)
    profanity_tfidf_features = get_features(author_list_train, profanity_tfidf.tf_idf, "Profanity TF_IDF")

    words_tfidf = fit_word_embeds_tfidf(author_list_train,authors_document=False)
...
def cross_validate(X, y, split_n = 7, rdmforest=True, one_nn=True, three_nn=True,
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X)):
        ...
        print(f"(split: {i+1}/{split_n}) Train: ", len(train_index), "Test: ", len(test_index))
        if i > 0:
            print(f"Split took {stop_w.split(as_string=True)}")
        print(f"Train Ratio of Labels (split: {i+1}/{split_n}): I: {ratio_train_i} | NI:{ratio_train_ni}")
        print(f"Test Ratio of Labels (split: {i+1}/{split_n}) : I: {ratio_test_i} | NI:{ratio_test_ni}")
