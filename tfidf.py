...
class tfidf:
    def __init__(self, corpus, terms_filter=None, lowercase=False) -> None:
        """
        ...
        self.idf_matrix = np.zeros((self.n_documents, len(self.idf_terms.keys())))
            ...
            for tweet in author:
                for w in set(tokenize_tweet(tweet, as_list=True)):
                    if w in self.idf_terms:
                        self.idf_matrix[document_i, self.idf_terms[w]] = 1
                document_i += 1
    ...
    def tf(self, author):
        ...
        tf_matrix = np.zeros((len(author), len(self.idf_terms.keys())))
